...
void main()
{
    ...
    // Ambient component
    vec3 ambient = u_Light.Color * u_Material.Ka;
    
    // Diffuse component
    float cosineTheta = max(0.0, dot(normal, lightDirection));
    vec3 diffuse = u_Light.Color * cosineTheta * u_Material.Kd;
    
    // Specular component
    float specularStrength = 0.5;
    
    float cosinePhi = max(0.0, dot(viewDirection, reflectionDirection));
    vec3 specular = u_Light.Color * pow(cosinePhi, u_Material.Shininess) * u_Material.Ks;
